...
//        //dp i j 总金额 j的情况下 i 范围内的物品 可选范围有多少种组合，注意不是钱币的数量
//        vector<vector<double>> dp(coins.size() + 1, vector<double>(amount+1));
//        int maxSize = coins.size();
...
//            }
//        }
//        return dp[maxSize][amount];
//    }
//};


/*valid 数组在这段代码中用于提前判断目标金额 amount 是否可以被 coins 组合出来，避免不必要的后续计算。*/
class Solution {
public:
    int change(int amount, vector<int>& coins) {
...
int main()
{
    ...
    cout << so.change(4681,coins) << endl;
    return 0;
}